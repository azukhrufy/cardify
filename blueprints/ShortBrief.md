...
1. Phase 1 : it will develop initial component of the website, Header Menu, Footer, Hero Section, and another section in Landing page that define who we are. It will contain about Typography, Tagline, and content of landing page. The page runs dark → light → dark: Header and Hero on near-black, the "Who We Are" section on white, Footer back on near-black, with a 1px brand-gradient hairline at the seam. Stack is fixed here: Next.js Pages Router, React 19, and Chakra UI v2 (theme tokens in `theme.js` — no Tailwind), so Phase 2 tabs inherit the same design system. Including code about web SEO — brand name, domain, and metadata all locked to `Cardify` / `cardify.my.id` so the brand is indexed under one identity from day one.
